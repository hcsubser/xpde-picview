...
void on_edit_file( GtkWidget* btn, MainWin* mw );
...
void create_nav_bar( MainWin* mw, GtkWidget* box )
{
    ...
    add_nav_btn( mw, PACKAGE_ICONS_DIR "/previous.png", _("Previous"), G_CALLBACK(on_prev), FALSE );
    add_nav_btn( mw, PACKAGE_ICONS_DIR "/next.png", _("Next"), G_CALLBACK(on_next), FALSE );
...
#endif
    add_nav_btn( mw, PACKAGE_ICONS_DIR "/fullscreen.png", _("Full Screen"), G_CALLBACK(on_full_screen), FALSE );   // gtk+ 2.8+
    mw->btn_fit = add_nav_btn( mw, PACKAGE_ICONS_DIR "/fit_to_size.png", _("Fit Image To Window Size"),
                           G_CALLBACK(on_zoom_fit), TRUE );
    mw->btn_play_stop = add_nav_btn_img( mw, PACKAGE_ICONS_DIR "/slideshow.png", _("Start Slideshow"), G_CALLBACK(on_slideshow), TRUE, &mw->img_play_stop );

	gtk_box_pack_start( (GtkBox*)mw->nav_bar, gtk_vseparator_new(), FALSE, FALSE, 0 );

    add_nav_btn( mw, PACKAGE_ICONS_DIR "/zoom_in.png", _("Zoom In"), G_CALLBACK(on_zoom_in), FALSE );
    add_nav_btn( mw, PACKAGE_ICONS_DIR "/zoom_out.png", _("Zoom Out"), G_CALLBACK(on_zoom_out), FALSE );
...
//    gtk_box_pack_start( (GtkBox*)nav_bar, percent, FALSE, FALSE, 2 );
	...
	mw->btn_rotate_cw = add_nav_btn( mw, PACKAGE_ICONS_DIR "/rotate_clockwise.png", _("Rotate Clockwise"), G_CALLBACK(on_rotate_clockwise), FALSE );
    mw->btn_rotate_ccw = add_nav_btn( mw, PACKAGE_ICONS_DIR "/rotate_counterclockwise.png", _("Rotate Counterclockwise"), G_CALLBACK(on_rotate_counterclockwise), FALSE );
    
    gtk_box_pack_start( (GtkBox*)mw->nav_bar, gtk_vseparator_new(), FALSE, FALSE, 0 );

    add_nav_btn( mw, PACKAGE_ICONS_DIR "/delete.png", _("Delete File"), G_CALLBACK(on_delete), FALSE );
    add_nav_btn( mw, PACKAGE_ICONS_DIR "/save.png", _("Save File"), G_CALLBACK(on_save), FALSE );
	add_nav_btn( mw, PACKAGE_ICONS_DIR "/edit_in_paint.png", _("Edit File"), G_CALLBACK(on_edit_file), FALSE );

	gtk_box_pack_start( (GtkBox*)mw->nav_bar, gtk_vseparator_new(), FALSE, FALSE, 0 );

	add_nav_btn( mw, PACKAGE_ICONS_DIR "/about.png", _("Edit File"), G_CALLBACK(on_about), FALSE );
    /*gtk_box_pack_start( (GtkBox*)mw->nav_bar, gtk_vseparator_new(), FALSE, FALSE, 0 );
...
gboolean main_win_open( MainWin* mw, const char* file_path, ZoomMode zoom )
{
    ...
    if( zoom == ZOOM_NONE )
    {
        ...
        area.x = 0;
		area.y = 0;
		area.width = 1680;//gdk_screen_width();
		area.height = 1050;
...
void on_save_as( GtkWidget* btn, MainWin* mw )
{
    ...
    /*gchar *file = {image_list_get_current_file_path(mw->img_list),"\0"};//, *type;
    pid_t pid = fork();
    if (0  == pid) {
		execlp("/usr/bin/gnome-paint", "/usr/bin/gnome-paint", file, (char*) NULL);
		//printf("r =    errno = %d",errno);
    	exit(EXIT_FAILURE);
    }*/
    ...
    if( ! mw->pix )
        return;
	gchar *file, *type;
    printf("%s",image_list_get_dir(mw->img_list));
		printf("\n%s",image_list_get_current_file_path(mw->img_list));
    printf("\n%s",image_list_get_current( mw->img_list ));
    fflush(stdout);
...
void on_zoom_in( GtkWidget* btn, MainWin* mw )
{
    ...
    //gtk_widget_set_halign( gtk_bin_get_child( (GtkBin*)mw->scroll ),50);
    //gtk_widget_set_halign(mw->img_view,50);
   // gtk_widget_set_halign(mw->scroll,50);
}
