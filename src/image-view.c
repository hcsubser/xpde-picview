...
void image_view_set_scale( ImageView* iv, gdouble new_scale, GdkInterpType type )
{
    ...
    if( iv->pix )
    {
        ...
        if(hadj_value>0)
        	gtk_adjustment_set_value(iv->hadj, hadj_value);
        else
        	gtk_adjustment_set_value(iv->hadj,0);
        //iv->vadj->value = vadj_value > 0 ? vadj_value : 0;
        if(vadj_value>0)
        	gtk_adjustment_set_value(iv->vadj, vadj_value);
        else
        	gtk_adjustment_set_value(iv->vadj,0);
